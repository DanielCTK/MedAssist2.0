    ...
    <div className={`h-screen w-full font-sans overflow-hidden transition-colors duration-500 ${isDarkMode ? 'bg-black text-slate-100' : 'bg-slate-50 text-slate-900'}`}>
      <AnimatePresence mode="wait">
        {!currentUser ? (
          ...
          >
            {/* onEnter is effectively handled by firebase auth listener now, but we keep prop for manual triggers if needed */}
            <LandingPage onEnter={() => {}} />
          </motion.div>
        ) : (
          <motion.div 
                  ...
                  <div className="flex items-center space-x-3">
                      ...
                      <div className={`flex items-center space-x-2 border-l pl-3 cursor-pointer group ${isDarkMode ? 'border-slate-800' : 'border-slate-200'}`}>
                          <div className="relative">
                            <img 
                                src={currentUser.photoURL || "https://images.unsplash.com/photo-1612349317150-e413f6a5b16d?q=80&w=2070&auto=format&fit=crop"}
