...
const DiagnosisView: React.FC<DiagnosisViewProps> = ({ isDarkMode }) => {
  ...
  return (
    <div className="h-full flex flex-col md:flex-row gap-6 pb-6 overflow-y-auto">
      ...
      >
        <div className={`p-6 rounded-2xl border flex flex-col flex-1 shadow-lg ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'} ${hoverEffect}`}>
          ...
          {/* Main Image Area - Added min-h-[350px] for mobile visibility */}
          <div 
            className={`w-full min-h-[350px] md:min-h-0 md:flex-1 rounded-xl border-2 border-dashed relative overflow-hidden transition-all group ${
                imagePreview 
                ? 'border-transparent' 
