...
const Sidebar: React.FC<SidebarProps> = ({ currentView, setView, isDarkMode, onLogout }) => {
  ...
  return (
    <div className={`w-52 flex flex-col h-screen fixed left-0 top-0 border-r z-30 transition-colors duration-500 ${baseClasses}`}>
      ...
      <div 
        className={`p-5 border-b cursor-pointer ${isDarkMode ? 'border-slate-800 hover:bg-slate-900' : 'border-slate-100 hover:bg-slate-50'} transition-colors`}
      ...
      {/* Footer - Moved Logout to Settings page, only Settings link remains here */}
