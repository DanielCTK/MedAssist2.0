...
import { Moon, Sun, Bell, Search, Menu, ChevronDown, Stethoscope, Activity, Globe } from 'lucide-react';
...
// ==================================================================================
// 🖼️ ASSETS CONFIGURATION (APP BACKGROUND)
// ==================================================================================
const appBgImage = "https://images.unsplash.com/photo-1579684385127-1ef15d508118?q=80&w=2080&auto=format&fit=crop"; 

// 🇯🇵 JAPANESE PATTERN: ASANOHA (Hemp Leaf)
...
const App: React.FC = () => {
  ...
  // Toggle Body class for global styles if needed
  useEffect(() => {
    if (isDarkMode) {
      ...
      <AnimatePresence mode="wait">
        {!hasEntered ? (
          ...
          >
            ...
            {/* ml-52 matches the new Sidebar width */}
            <main className="flex-1 ml-52 h-full relative flex flex-col">
               {/* TOP HEADER BAR - Compact Height h-12 */}
               <header className={`h-12 px-5 flex items-center justify-between border-b transition-colors z-20 ${isDarkMode ? 'bg-black/80 border-slate-800' : 'bg-white border-slate-200'}`}>
                  ...
                  <div className="flex items-center space-x-3">
                      ...
                      {/* DOCTOR PROFILE SECTION - Compact */}
                      <div className={`flex items-center space-x-2 border-l pl-3 cursor-pointer group ${isDarkMode ? 'border-slate-800' : 'border-slate-200'}`}>
                          ...
                          <div className="hidden lg:block text-right">
                              <p className={`text-[10px] font-bold leading-none mb-0.5 group-hover:text-red-500 transition-colors ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
                                Dr. Robert Fox
                              </p>
               ...
               {/* MAIN CONTENT AREA - Reduced padding p-3 */}
               <div className={`flex-1 overflow-y-auto custom-scrollbar relative p-3 ${isDarkMode ? 'bg-black' : 'bg-slate-50'}`}>
                   ...
                   <AnimatePresence mode="wait">
                      ...
                      >
                        {currentView === 'dashboard' && <Dashboard isDarkMode={isDarkMode} />}
